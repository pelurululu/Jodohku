...
async function doPayment(tier, price) {
  ...
  openOv('ov-pay-proc');
  try {
    const r = await apiCall('POST', '/payment/demo-upgrade', { user_uid: S.uid, tier, amount: price });
    if (r.ok) {
...
async function loadPioneer() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // Small live decrement to show activity — stays within real remaining range
      setInterval(() => {
        if (Math.random() > .78 && c > 0) { c--; if (el) el.textContent = c.toLocaleString(); }
      }, 11000);
    } else {
