...
async function doPayment(tier, price) {
  ...
  openOv('ov-pay-proc');
  try {
    const r = await apiCall('POST', '/payment/create-bill', { user_uid: S.uid, tier, amount: parseFloat(price) });
    if (r.ok) {
      const d = await r.json();
      closeOv('ov-pay-proc');
      if (d.payment_url) { window.location.href = d.payment_url; return; }
      // Fallback if no payment_url (e.g. staging env returns tier directly)
      if (d.tier) { S.tier = d.tier; S.premium = d.is_premium; S.msgCount = 0; }
      unlockTier(tier, d.expires_days || (tier === 'Sovereign' ? 365 : 30));
...
function updateNavBadge() {
  ...
  b.textContent = cleanTierName(S.tier);
...
function refreshProfile() {
  ...
  if (wCard) {
    if (S.wingmanVerdict || (S.occupation && S.education)) {
      ...
      document.getElementById('prof-wingman-text').textContent = S.wingmanVerdict || wingmanAnalysis(S.occupation || 'Lain-lain', S.education || 'Degree', S.status || 'Bujang');
...
async function submitIC() {
  ...
  btn.disabled = true; if (sp) sp.style.display = 'inline-block'; hideMsg('ic-status-msg');
  try {
    const r = await apiCall('POST', '/profile/upload-ic', { user_uid: S.uid, ic_image: S._icData.split(',')[1] });
    if (r.ok) {
      S.icVerified = 'pending'; save(); refreshProfile();
  ...
  // Fallback — mark pending even if API unreachable
  S.icVerified = 'pending'; save(); refreshProfile();
  setMsg('ic-status-msg', '✅ IC diterima! Status: Dalam semakan.', 's');
  setTimeout(() => { closeOv('ov-ic'); toast('IC dihantar untuk semakan! ✅', 'ok', 4000); }, 1800);
  if (sp) sp.style.display = 'none'; btn.disabled = false;
}
...
// ══════════ IC UPLOAD ══════════
async function muatNaikIC(inputId) {
  const fileInput = document.getElementById(inputId);
  if (!fileInput || !fileInput.files.length) { toast('Sila pilih fail IC dahulu.', 'err'); return; }
  toast('Memuat naik IC... sila tunggu.', 'info', 4000);
  try {
    const compressed = await compressImage(fileInput.files[0]);
    const formData = new FormData();
    formData.append('ic_document', compressed);
    formData.append('user_uid', S.uid || '');
    const r = await fetch(API + '/profile/upload-ic', {
      method: 'POST',
      headers: S.tok ? { 'Authorization': 'Bearer ' + S.tok } : {},
      body: formData
    });
    if (r.ok) {
      S.icVerified = 'pending'; save();
      toast('IC berjaya dihantar! Pengesahan dalam 24 jam. ✅', 'ok', 5000);
    } else {
      toast('Muat naik IC gagal. Cuba lagi.', 'err');
    }
  } catch (e) {
    console.error('IC upload error:', e);
    toast('Tidak dapat menghubungi pelayan.', 'err');
  }
}

// ══════════ SECURE PHOTO UPLOAD WITH COMPRESSION ══════════
async function muatNaikGambarPantas(inputId) {
  const fileInput = document.getElementById(inputId);
  ...
  try {
    ...
    formData.append('user_uid', S.uid || '');
    const r = await fetch(API + '/user/' + S.uid + '/upload-photo', {
      method: 'POST',
      headers: S.tok ? { 'Authorization': 'Bearer ' + S.tok } : {},
      body: formData
    });
    if (r.ok) {
      const d = await r.json();
      if (d.photo_url) { S.photo = d.photo_url; save(); updateNavAvatar(); }
      toast('Gambar berjaya dimuat naik! ✅', 'ok');
    }
  } catch (e) {
    console.error('Photo upload error:', e);
    toast('Gagal muat naik gambar.', 'err');
  }
}
