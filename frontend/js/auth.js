...
async function saveExtendedProfile() {
  ...
  } catch { /* Teruskan walaupun API gagal — data sudah dalam localStorage */ }
...
async function doOTP() {
  const email = document.getElementById('l-ph').value.trim();
  if (!email || !email.includes('@')) { setMsg('l-m1', 'Sila masukkan alamat emel yang sah.', 'e'); return; }
  setBL('l-b1t', 'l-b1s', true); hideMsg('l-m1');
  try {
    // Find phone by email first, then request OTP
    const r = await apiCall('POST', '/auth/email/request-otp-by-email', { email });
    if (!r.ok) {
      const err = await r.json();
      setMsg('l-m1', err.detail || 'Emel tidak dijumpai. Sila daftar dahulu.', 'e');
  ...
  document.getElementById('l-phshow').textContent = email;
  ...
  setMsg('l-m2', 'Kod OTP telah dihantar ke emel anda.', 'i');
...
async function doVerify() {
  ...
  if (!otp || otp.length !== 6) { setMsg('l-m2', 'Sila masukkan kod OTP 6 digit.', 'e'); return; }
  setBL('l-b2t', 'l-b2s', true); hideMsg('l-m2');
  try {
    const r = await apiCall('POST', '/auth/email/verify-otp-by-email', { email, otp_code: otp });
    if (r.ok) {
      const d = await r.json();
      S.uid = d.user_id; S.tok = d.access_token; S.tier = d.tier || 'Silver';
      S.name = d.name || 'Pengguna'; S.msgCount = d.msg_count || 0;
      save(); setBL('l-b2t', 'l-b2s', false);
      toast('Selamat kembali! 👋', 'ok'); finishLogin(); return;
    }
    const err = await r.json();
    setMsg('l-m2', err.detail || 'Kod OTP tidak sah.', 'e');
  } catch {
    setMsg('l-m2', 'Tidak dapat menghubungi pelayan.', 'e');
  }
