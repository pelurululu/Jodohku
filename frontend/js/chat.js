// ══════════ REPLIES ══════════
const REPLIES_MS = [
  'Waalaikumussalam. Terima kasih sudi menghubungi saya 😊',
  'InsyaAllah, boleh ceritakan lebih lanjut tentang diri anda?',
  'Saya setuju dengan pandangan anda. Kita ada banyak persamaan.',
  'Menarik! Saya pun minat aktiviti yang sama.',
  'Alhamdulillah, senang berkenalan dengan anda. Ceritakan tentang keluarga anda?',
  'Apa pendapat anda tentang kehidupan selepas berkahwin?',
  'Saya harap kita boleh terus mengenali satu sama lain.',
  'Terima kasih kerana jujur. Saya sangat menghargainya.'
];
...
function useShortcut(btn) {
  ...
  inp.focus();
  // Hide ONLY this button (not the whole bar) — Wingman mechanic
...
function checkAdvanceEligibility(mid) {
  ...
  if (sorted.length >= 3) {
    ...
    const diff1 = (d1 - d2) / 864e5, diff2 = (d2 - d3) / 864e5;
    if (diff1 <= 1 && diff2 <= 1) { bar.style.display = 'flex'; }
  }
}
...
function sendMsg() {
  ...
  save();
  // Don't hide shortcuts — only the individual clicked one hides (see useShortcut)
  setTimeout(() => {
    const rep = REPLIES_MS[Math.floor(Math.random() * REPLIES_MS.length)];
    appendBub(rep, false);
    S.history[mid].push({ text: rep, me: false }); save();
    checkAdvanceEligibility(mid);
    // Blur trigger after limit
    if (!S.premium && S.msgCount >= FREE_MSGS) {
      setTimeout(() => triggerBlurPaywall(), 1000);
    }
...
function updateQuotaBar() {
  ...
  if (!S.premium) {
    ...
    if (el) {
      el.textContent = left;
      // Turn red when 3 or fewer remain
      el.style.color = left <= 3 ? 'var(--red)' : 'var(--gold)';
    }
  } else {
    bar.style.display = 'none';
  }
...
// ══════════ BLUR PAYWALL (The Wall of Frustration & Desire) ══════════
function triggerBlurPaywall() {
  ...
  if (msgs) msgs.style.filter = 'blur(7px) grayscale(40%)';
  if (msgs) msgs.style.pointerEvents = 'none';
  if (shortcuts) shortcuts.style.filter = 'blur(7px)';
  if (shortcuts) shortcuts.style.pointerEvents = 'none';
  if (inputWrap) inputWrap.style.opacity = '0.3';
  if (inputWrap) inputWrap.style.pointerEvents = 'none';
  openOv('ov-pay');
}
