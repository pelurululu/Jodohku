...
// ══════════ CHAT HISTORY LOADER ══════════
async function muatTurunSejarahSembang(matchId) {
  if (!matchId || !S.tok) return;
  try {
    const r = await apiCall('GET', '/chat/' + matchId + '/messages');
    if (!r.ok) return;
    const data = await r.json();
    const msgs = data.messages || [];
    if (!msgs.length) return;
    // Populate history from server
    S.history[matchId] = msgs.map(m => ({
      text: m.message_text,
      me: m.sender_uid === S.uid,
      admin: m.is_system || false
    }));
    save();
    // Re-render if currently viewing this chat
    if (S.activeMatch && S.activeMatch.id === matchId) {
      const area = document.getElementById('cw-msgs');
      if (area) {
        area.innerHTML = '';
        S.history[matchId].forEach(x => appendBub(x.text, x.me, x.admin));
        area.scrollTop = area.scrollHeight;
      }
    }
    // Check paywall
    const userMsgs = S.history[matchId].filter(m => m.me).length;
    if (userMsgs >= (S.premium ? 999 : 10)) triggerBlurPaywall();
  } catch (e) {
    console.error('Gagal muat sejarah sembang:', e);
  }
}
