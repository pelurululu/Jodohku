...
async function doAction(cid, action, idx) {
  ...
  setTimeout(() => {
    if (el) el.remove();
    if (!document.getElementById('f-cards').children.length) {
      document.getElementById('f-cards').style.display = 'none';
      document.getElementById('f-lock').style.display = 'block';
    }
  }, 280);
  try {
    ...
    if (r.ok) {
      const d = await r.json();
      if (d.status === 'MATCHED') { showMatch(d.match_data); return; }
    }
  } catch {}
  // No fallback — if API fails, action is silently dropped (will retry on next feed load)
}

function showMatch(matchData) {
  if (!matchData) return;
  document.getElementById('m-name').textContent = matchData.name || 'Padanan Baru';
  document.getElementById('m-score').textContent = (matchData.score || 0) + '%';
  document.getElementById('m-verdict').textContent = matchData.verdict || 'Keserasian tinggi berdasarkan analisis psikometrik anda.';
  const sc = matchData.score || 0;
  const mbdg = document.getElementById('m-match-badge');
  mbdg.innerHTML = sc >= 90 ? '<span class="bmatch-best">⭐ PADANAN TERBAIK</span>' : '<span class="bmatch-rec">✓ DISYORKAN</span>';
  const match = { id: matchData.match_id, name: matchData.name, score: sc, photo: matchData.photo || null, traits: matchData.traits || [] };
  S.activeMatch = match;
