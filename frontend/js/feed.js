...
async function doAction(cid, action, idx) {
  ...
  try {
    ...
    if (r.ok) {
      const d = await r.json();
      if (d.status === 'MATCHED') { showMatch(d); }
    }
  } catch {}
}

function showMatch(d) {
  // d = { status, match_id, score, match_data: { match_id, name, score, verdict, photo, traits } }
  ...
  mbdg.innerHTML = score >= 90 ? '<span class="bmatch-best">⭐ PADANAN TERBAIK</span>' : '<span class="bmatch-rec">✓ DISYORKAN</span>';
  const match = { id: mid, name, score, photo, traits };
  S.activeMatch = match;
  if (!S.matches.find(m => m.id === mid)) { S.matches.push(match); save(); }
