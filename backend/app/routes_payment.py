...
async def create_bill(data: PaymentCreateBody, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "note": "[DEMO] ToyyibPay belum disambung. Guna /payment/demo-upgrade untuk testing.",
    }
...
# ═══════════════════════════════════════════
#  POST /payment/demo-upgrade — [DEMO] Naik taraf terus
# ═══════════════════════════════════════════
@router.post("/payment/demo-upgrade")
async def demo_upgrade(data: PaymentCreateBody, bg: BackgroundTasks, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.uid == data.user_uid).first()
    if not user:
        raise HTTPException(404, detail="Pengguna tidak ditemui.")
